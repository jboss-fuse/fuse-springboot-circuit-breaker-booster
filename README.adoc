...
* A _greetings service_ that invokes the name service to get a name and then returns the string, `Hello, NAME`.

This booster demonstrates inserting the Hystrix circuit breaker between the greetings service and the name service.
If the name service becomes unavailable, the circuit breaker instructs the greetings service to follow an alternative behavior and respond to the client immediately, thereby avoiding a blockage during the the time that it takes for the name service to restart.
...
. Change the directory to the folder that contains the extracted booster application (for example, `my_openshift/fuse-springboot-circuit-breaker-booster-master`) :
+
[source,bash,options="nowrap",subs="attributes+"]
----
$ cd my_openshift/fuse-springboot-circuit-breaker-booster-master
----
